/*
 * @(#)RTFGenerator.java	1.13 03/12/19
 *
 ...
 *
 *
 * THIS IS A WORKAROUND FOR BUG # 4375404
 *
 * Since Sun is not willing to fix it, we have to copy the source code.
 * Can't help it...
 *
 *
...
static public void writeDocument(Document d, OutputStream to)
    throws IOException
{
	writeDocument(d,to, 0,d.getLength());
}

static public void writeDocument(Document d, OutputStream to, int pos, int length)
	throws IOException
{
    ...
    for(int idx = 0; idx < max; idx++) {
	    ...
	    int p0 = Math.max(pos,elem.getStartOffset());
	    int p1 = Math.min(pos+length,elem.getEndOffset());
	    if (p1 > p0)
			gen.writeParagraphElement(elem, p0, p1-p0);
    }
...
public void writeParagraphElement(Element el, int offset, int length)
    throws IOException
{
    ...
    {
	    Element sub = el.getElement(idx);
	    int p0 = Math.max(offset, sub.getStartOffset());
	    int p1 = Math.min(offset+length, sub.getEndOffset());
	    if (p1 > p0)
			writeTextElement(sub, p0, p1-p0);
    }

	if ((offset+length) >= el.getEndOffset())
...
public void writeTextElement(Element el, int offset, int length)
    throws IOException
{
    ...
    if (el.isLeaf()) {
	try {
	    el.getDocument().getText(offset, length, this.workingSegment);
