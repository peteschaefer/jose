...
/**
 * a hash map that maps long keys to int values
 *
 * -----------------------------------------------------------------------------------------------------------
 * there are two integers that must not be used as hash keys or values:
        ...
        implements Map
{
	 ...
	 */
	public LongIntMap(int capacity, float loadFactor)
	{
	...
	{
		int result = put(((Number)key).longValue(), ((Number)value).intValue());
		if (result==NOT_FOUND)
			return null;
		else
	...
	{
		int value = get(((Number)key).longValue());
		if (value==NOT_FOUND)
			return null;
		else
	...
	{
		int result = remove(((Number)key).longValue());
		if (result==NOT_FOUND)
			return null;
		else
			return new Integer(result);
	 ...
	 */
	public final int putAll(long[] keys, int[] values)
	{
		int result = 0;
	...
	public final void putAll(Map map)
	{
		if (map instanceof LongIntMap) {
			...
			LongIntMap imap = (LongIntMap)map;
			putAllInternal(imap.fKeys,imap.fValues);
			return;
		}
/*
		if (map instanceof LongHashMap) {
			//	performance shortcut for IntHashMap
			LongHashMap imap = (LongHashMap)map;
			putAllInternal(imap.fKeys,imap.fValues);
			return;
		}
*/
	 ...
	 */
	public final int removeAll(long[] keys)
	{
		int result = 0;
		for (int i=keys.length-1; i>=0; i--)
		{
			if (remove(keys[i]) != NOT_FOUND)
				result++;
		}
		return result;
	}
	...
	{
		if (coll instanceof LongMap) {
			//	performance shortcut for IntMaps
			return removeAllInternal(((LongMap)coll).fKeys);
		}

		int result = 0;
		java.util.Iterator i = coll.iterator();
		while (i.hasNext())
			if (remove(((Number)i.next()).longValue()) != NOT_FOUND)
				result++;
		return result;
	}
	...
	{
		return new LongIntMapValueCollection();
	}
	...
	protected final void rehash(int newCapacity)
	{
		...
		{
			long key = oldKeys[j];
			if (key==0L || key==DELETED_KEY) continue;	//	empty
	...
	/**
	 * used for bulk copies from IntHashMap
	 * 0 and deleted keys ignored
	 */
	protected final int putAllInternal(long[] keys, Object[] values)
	{
		int result = 0;
		for (int i=keys.length-1; i>=0; i--)
		{
			if (keys[i]==0L || keys[i]==DELETED_KEY)
				continue;
			int ivalue = (values[i]==null) ? 0:((Number)values[i]).intValue();
			if (put(keys[i],ivalue) != NOT_FOUND)
				result++;
		}
		return result;
	}
	 ...
	 * Copies all of the mappings from the specified arrays to this map.
	 * The effect of this call is equivalent to that
	 * of calling {@link #put(long,int) put(k, v)} on this map once
	 * for each mapping from key <tt>k</tt> to value <tt>v</tt> in the
	 * specified arrays.  The behavior of this operation is unspecified if the
	 * specified map is modified while the operation is in progress.
	 *
	 ...
	 * @throws java.lang.ArrayIndexOutOfBoundsException if values is shorter than keys
	 */
	public final int putAllInternal(long[] keys, int[] values)
	{
		int result = 0;
		for (int i=keys.length-1; i>=0; i--)
		{
	 ...
	 * @see    #remove(java.lang.Object)
	 */
	public final int removeAllInternal(long[] keys)
	{
		int result = 0;
		for (int i=keys.length-1; i>=0; i--)
		{
	 ...
	 */
	public final class ValueIterator extends LongMap.Iterator
	{
	 ...
	 */
	public final class EntryIterator extends LongMap.Iterator
	{
	 ...
	 * */
	public class LongIntMapKeySet extends LongMap.KeySet implements Set
	{
		/**
		 * @see de.jose.util.map.IntIntMap#remove(java.lang.Object)
		 */
		public boolean remove(Object obj) {
			return LongIntMap.this.remove(((Number)obj).longValue()) != NOT_FOUND;
		}
...
//	TODO	return IntIntMap.retainAllKeys(coll);
		...
		/**
		 * @see de.jose.util.map.IntIntMap#removeAll(java.util.Collection)
		 */
		public boolean removeAll(Collection coll) {
			return LongIntMap.this.removeAll(coll) > 0;
		}

		/**
		 * @see de.jose.util.map.IntIntMap#keyIterator()
		 */
		public java.util.Iterator iterator()
		{
			return LongIntMap.this.keyIterator();
		}
	}
	 ...
	 */
	public class LongIntMapValueCollection extends LongMap.AbstractSet implements Collection
	{
		/**
		 * @see de.jose.util.map.IntHashMap#removeValue(java.lang.Object)
		 */
		public boolean remove(Object obj) {
			return LongIntMap.this.removeValue(obj);
		}
		 ...
		 */
		public java.util.Iterator iterator() {
			return LongIntMap.this.valueIterator();
		}
		...
		public Object[] toArray() {
			return LongIntMap.this.valueArray((Object[]) null);
		}
		...
		public Object[] toArray(Object[] array) {
			return LongIntMap.this.valueArray(array);
		}


		protected int[] toIntArray()
		{
			return LongIntMap.this.fValues;
		}
	}
	 ...
	 */
	public class LongIntMapEntrySet extends LongMap.AbstractSet implements Set
	{
		/**
		 * @see de.jose.util.map.IntIntMap#entryIterator()
		 */
		public java.util.Iterator iterator()
		{
			return LongIntMap.this.entryIterator();
		}
	}
}
